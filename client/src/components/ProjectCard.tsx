...
const ProjectCard = ({gen, forCommunity = false} :
    {gen: Project, setGenerations: React.Dispatch<SetStateAction<Project[]>>, forCommunity?: 
    boolean}) => {
         ...
         <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
            ...
            {!forCommunity && (
                ...
                group-hover:opacity-100 transition flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-end w-32 text-sm">
                        ...
                        50 rounded-lg shadow-md mt-2 py-1 z-10`}>
                            ...
                            onClick={()=> navigator.share({url: gen.
                                generatedVideo || gen.generatedImage, title: gen.
                                productName, text: gen.productDescription})}
                                className="w-full flex gap-2 items-center px-4 py-2
                                hover:bg-black/10 cursor-pointer">
                                    <Share2Icon size={14}/> Share
                                </button>}

                                <button onClick={()=> handleDelete(gen.id)} className="w-full
            ...
            <div className="absolute right-3 bottom-3 flex items-center">
                <img 
                    ...
                    alt="product" 
                    className="w-16 h-16 object-cover rounded-full animate-float border-2 border-white bg-white" 
                ...
                {gen.uploadedImages[1] && (
                    <img 
                        ...
                        alt="model" 
                        className="w-16 h-16 object-cover rounded-full animate-float -ml-8 border-2 border-white bg-white" 
                        style={{objectFit: 'cover', animationDelay: '3s'}}
                    />
                )}
            </div>
